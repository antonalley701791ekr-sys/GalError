...
CoverCropper.prototype.confirm=function(){
 ...
 fetch(this.uploadUrl,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'image_data='+encodeURIComponent(base64Data)}).then(function(r){return r.json();}).then(function(data){if(data.success){var input=document.getElementById(self.croppedPathInput);if(input)input.value=data.path;if(typeof self.onCropped==='function')self.onCropped(data.path,base64Data);self._closeModal();}else{alert('上传失败: '+(data.message||'未知错误'));}}).catch(function(e){alert('上传请求失败: '+e.message);}).finally(function(){self.confirmBtn.disabled=false;self.confirmBtn.textContent=self.confirmText;});
};
