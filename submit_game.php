...
if ($editId > 0) {
    ...
    $isEditMode = true;
    $isOriginalSubmitter = !empty($editGame['user_id']) && (int)$editGame['user_id'] === (int)getCurrentUserId();
        ...
        if ($existing && (!$isEditMode || (int)$existing['id'] !== (int)$editId)) {
            ...
            if ($messageType !== 'error') {
                    ...
                    if ($canDirectEditGame) {
                        ...
                        if ($result) {
                            $pdo->prepare("INSERT INTO game_revisions (game_id, user_id, user_ip, old_data, new_data, status, created_at) VALUES (?, ?, ?, ?, ?, 'approved', NOW())")
                                ->execute([$editId, getCurrentUserId(), getClientIP(), json_encode($oldGameData, JSON_UNESCAPED_UNICODE), json_encode($newGameData, JSON_UNESCAPED_UNICODE)]);
                            $message = '游戏信息修改已直接生效。';
                            $messageType = 'success';
                            $_POST = [];
                        } else {
                        ...
                        $result = $stmt->execute([$editId, getCurrentUserId(), getClientIP(), json_encode($oldGameData, JSON_UNESCAPED_UNICODE), json_encode($newGameData, JSON_UNESCAPED_UNICODE)]);
                        if ($result) {
                ...
                $result = $stmt->execute([
                    ...
                    $releaseDate ?: null, $coverImage, $vndbCoverUrlValue, $platforms, $submitStatus, getClientIP(), getCurrentUserId()
                ]);
                    ...
                    <form method="post" enctype="multipart/form-data" id="gameForm">
                        ...
                        <div class="form-group">
                            ...
                            <div id="cover_file_wrap" style="display: none;">
                                <input type="file" name="cover_file" class="form-input" id="cover_file" accept="image/jpeg,image/png" onchange="onFileSelected(this)">
