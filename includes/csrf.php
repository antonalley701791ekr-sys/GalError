...
if (!function_exists('csrf_validate_request_flexible')) {
    function csrf_validate_request_flexible(array $scopes = ['admin_form', 'default'], string $field = '_csrf'): bool {
        $token = $_POST[$field] ?? '';
        if (!is_string($token) || $token === '') {
            $token = (string)($_SERVER['HTTP_X_CSRF_TOKEN'] ?? '');
        }
        ...
        foreach ($scopes as $scope) {
            if (csrf_validate($token, (string)$scope)) {
                return true;
            }
        }
        return false;
