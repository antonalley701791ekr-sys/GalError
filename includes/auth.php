...
function renderAdminSidebar($currentPage) {
    ...
    $badgeRow = $pdo->query("SELECT
        ...
        (SELECT COUNT(*) FROM errors WHERE status='pending') +
        (SELECT COUNT(*) FROM error_revisions WHERE status='pending') as pending_errors
    ")->fetch();
    $badgeCounts = [
    ...
    $menuItems = [
        ...
        ['file' => 'migrate_covers.php', 'label' => '封面迁移', 'module' => null, 'super_only' => true],
    ...
    foreach ($menuItems as $item) {
        ...
        echo '<li><a href="' . $item['file'] . '"' . $active . '>' . $icon . '<span>' . htmlspecialchars($item['label'], ENT_QUOTES, 'UTF-8') . '</span>' . $badge . '</a></li>';
    }
    echo '</ul>';
    echo '</div>';
    echo '</aside>';
