...
function handleCroppedCoverData($base64Data, $target = 'covers') {
    ...
    if (!in_array($target, ['covers', 'avatars'], true)) {
        return ['success' => false, 'message' => '无效的图片用途'];
    }

    // 解析 data URI 格式: data:image/jpeg;base64,/9j/4AAQ...
    if (!preg_match('/^data:image\/(jpeg|png);base64,(.+)$/s', $base64Data, $matches)) {
        return ['success' => false, 'message' => '图片数据格式错误，仅支持 JPEG/PNG'];
    }

    $mimeType = $matches[1]; // jpeg 或 png
    ...
    if (strlen($imageData) > $maxSize) {
        return ['success' => false, 'message' => '裁剪后的图片大小超过 2MB 限制'];
    }
    ...
    // 确定文件扩展名
    $extension = ($mimeType === 'png') ? 'png' : 'jpg';
...
function handleCroppedAvatarData($base64Data) {
    return handleCroppedCoverData($base64Data, 'avatars');
}
